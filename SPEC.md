...
## Execution Mandate (Top Priority)

Execute this plan end-to-end with high rigor and no skipped foundations. Deliver a production-ready application with complete, reusable components, secure defaults, strong test coverage, and clear operational documentation.
Before building new features, audit existing code in the repository, review it for quality/security/architecture gaps, and improve it where needed.
...
* Use **EffectTS** for all backend APIs, service orchestration, and error handling (no ad-hoc backend error flow outside Effect patterns)
...
* Enforce security-by-default across architecture, code, tests, and reviews (validation, authz, rate limits, secure headers, safe logging, secret handling, dependency hygiene)
* Build in security, rate limits, background indexing, tests, and deployment prep (no deployment execution)
* Use a minimalist Windows 98-inspired frontend design language implemented with modular reusable components
* Reuse shared frontend primitives throughout the app; define exactly two button variants (`PrimaryButton`, `SecondaryButton`) and use only those styles consistently
* Ensure responsive design for desktop, tablet, and mobile platforms
* Keep `PROMPT.md` as the source-of-truth spec; maintain `TODO.md` as step-by-step execution checklist, `LEARNINGS.md` for failures/incidents and fixes, and `AGENT.md` for actionable agent instructions
* Always inspect and review existing code paths first, then implement incremental improvements before or alongside new feature work
* After every implemented feature or new component, run `git add` + `git commit` + `git push` immediately
* Never commit environment files, secrets, API keys, tokens, credentials, private documents, or any sensitive data
...
## 2.5 Required project docs (must exist and be maintained)

* `PROMPT.md`: canonical specification, constraints, architecture, and implementation instructions
* `TODO.md`: detailed step-by-step development checklist derived from `PROMPT.md`; update progress continuously
* `LEARNINGS.md`: append-only log for anything that goes wrong, root cause, fix, and prevention
* `AGENT.md`: concise agent operating instructions for execution quality, security, modularity, testing, and commit discipline

When conflicts occur, `PROMPT.md` is authoritative.

Before every push, review staged changes and ensure no sensitive files/values are included.
...
### 23.3 Frontend design system (mandatory)

* Minimalist Windows 98-inspired visual style across the app
* Build modular reusable UI primitives and compose pages from them
* Define and use only two shared button variants: primary and secondary
* Keep component behavior/style consistent across chat, corpus, and settings/admin screens
* Responsive layout and typography for desktop, tablet, and mobile breakpoints
* Verify responsive behavior in tests/review (no broken layouts, clipped text, or unusable controls)
...
* `docs/operations.md`
* root `Dockerfile` + `.dockerignore` for containerized deploy path
* optional `railway.json` (if needed)
* web and worker start commands documented

### Dockerfile setup (required for easy deployment)

Use the root `Dockerfile` and `.dockerignore` as the default container path for Railway/container deployments.

What the Docker setup should do:

* Build with Bun using multi-stage images (`deps`, `builder`, `runner`)
* Install dependencies with lockfile integrity (`bun install --frozen-lockfile`)
* Build production Next.js assets (`bun run build`)
* Run the production server with Bun (`bun run start`) on `0.0.0.0:3000`
* Keep image lean using `.dockerignore`
* Exclude `data/` from build context by default to prevent huge images and accidental corpus leakage
* Support runtime corpus access via mounted `data/` volume or external corpus source
* Never bake secrets into the image; inject secrets through runtime environment variables

Minimum runtime container expectations:
